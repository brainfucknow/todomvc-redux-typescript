...
- **A Gherkin step asserting an absence must not take its operand from an example column.** Mutating that cell leaves the assertion true, so the whole column is unkillable by construction and the scenario cannot fail for the reason it exists. Inline the literal, or assert something positive that implies the absence. Found twice: once in task 01 when choosing production-build markers, and once in task 02 when a parameterised package name produced six live survivors.
- **Coverage is not assertion.** Task 02 mutated `src/components` for the first time and found seven survivors in code the CRAP gate reported at 100% covered, among them three `propTypes` declarations enforced by nothing and `TodoTextInput`'s `.trim()`. A tier that runs the code without asserting on it reports the same number as one that checks it.
...
| 02 | Replace shallow-renderer tests with Testing Library | (file deleted) | done |
