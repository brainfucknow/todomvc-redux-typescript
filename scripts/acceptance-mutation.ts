...
import { ACCEPTANCE_IMPLEMENTATION_STAMP, gherkinManifest } from './mutation-reuse/layout.ts'
...
const implementationStamp = join(projectRoot, ACCEPTANCE_IMPLEMENTATION_STAMP)
...
const manifestPath = (slug: string): string => join(projectRoot, gherkinManifest(slug))
...
const reusable = resultsAreReusable(
  ...
  readIfPresent(implementationStamp),
...
if (reachedVerdict(results.map(({ status }) => status))) {
  mkdirSync(dirname(implementationStamp), { recursive: true })
  writeFileSync(implementationStamp, stampText(ACCEPTANCE_IMPLEMENTATION, currentImplementation))
}
